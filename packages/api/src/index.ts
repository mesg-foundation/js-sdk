...
const promisify = (client: any, method: string) => (request: any, credential?: Credential) => {
  const metadata = new Metadata()
  if (credential) {
    metadata.set('credential_username', credential.username)
    metadata.set('credential_passphrase', credential.passphrase)
  }
  return new Promise((resolve, reject) => client[method](request, metadata, (err: Error, res: any) => err ? reject(err) : resolve(res)))
}
