...
test('checkStreamReady() without error', function (t: Test) {
  t.plan(2);
...
test('checkStreamReady() with empty statuses', function (t: Test) {
  t.plan(1);
...
test('checkStreamReady() without having ready status', function (t: Test) {
  t.plan(1);
  t.equal(checkStreamReady({
      get(key) { return ['x'] }
